...
基于 Cloudflare Workers 的私人 Telegram 订阅查询机器人，支持订阅流量查询、节点统计、保存订阅、Mihomo 配置、原始订阅导出，以及机场和自建节点稳定性监测。
...
- `MONITOR_DB`: 保存监测目标和最近 30 天汇总样本的 D1 绑定。
...
- `/monitor`: 选择已保存机场或节点合集中的自建节点，开启、暂停或查看稳定性监测。
...
- 生成 Mihomo 配置与订阅链接
- 保存订阅
- 节点合集生成 Mihomo 订阅时，可勾选指定节点或一键导出全部节点
...
- 海创 VPS 上的 `monitor-agent/monitor_agent.py` 每 10 分钟领取用户主动开启的机场或自建节点任务。
- 每个节点只请求一次 Cloudflare 204 地址，不进行下载测速。
- Worker 负责 Bot 交互和鉴权，D1 保存目标级汇总；订阅 URL 和节点凭据不会写入 D1。
- 订阅接口临时失败时会使用上一次成功快照继续测试，并在结果中单独标明订阅接口异常。
- 自建节点当前复用节点合集的 VLESS 导出能力；非 VLESS 节点会显示为配置不支持，不会误报为掉线。
- 连续两次无可用节点才发送掉线提醒，连续两次恢复正常才发送恢复提醒。
- 探针超时或自身故障显示为“未知”，不计作节点离线。
- 每天北京时间 09:00 向开启了监测的用户推送健康报告。
